# FRAMELOG PRD v0.4
## FRAME//LOG — Product Requirements Document
...
FRAME//LOG is an Android application for film photographers to log shooting data in the field and manage equipment across sessions. Its core design principle is that the primary interaction — logging a frame — must be achievable one-handed, without unlocking the phone, in under three seconds.
...
*   Full database backup and restore (.framelog file)
...
*   Export backup (.framelog file via share sheet)
...
| --- | --- | --- | --- |
| OQ-1 | App name — FRAME//LOG is working title only | Branding, Play Store | Open — revisit before public release |
